...
`artifact/mod.rs`——二十格与落点规则。`Asset { category, name }`，一对名字：中文名与英文名。产物实例（某趟行程的报告与日志）落 `<产物落点>/<类别>/<工单名>.md`——产物落点由装载给（缺省 `<工作区根>/artifacts/`），不在工作区根下另猜（见 [workspace](workspace.md)·三处位置的分工）。
