...
`src/workspace/` 管端侧这一步：把命令行的位置落成实际路径，并决定路径怎么写给人看。工作区本身（模型与三件操作）在工具箱。
...
工作区的模型与三件操作在 `quanttide_work::workspace`：
...
这三件都要拿整个工作区才做得了，所以归工具箱；「在不在」由端侧判断——`workflow::check` 把 `exists` 传进去，工具箱不读文件系统。
