...
列出数据仓里的任务、各自跑哪条工作流、走过几步（进度条）、下一步是什么。
...
看步骤状态、开工、工作流、进度条、指令与产物路径、最近五条流水、下一步。进度条是十格加「走过/总数」的写法（如 `[███░░░░░░░] 1/3`）。运行上下文用任务里记的，`--root` / `--workflows` 可省。
...
走下一步。执行者是 agent 就把这一步交给 `pi` 跑，跑完核判据；执行者是 human 就提示轮到你。随后跑 rule 判据、交 agent 审、列 human 闸门、记一笔流水、写报告，末尾印进度条与下一步。所有步骤都走过则报「所有步骤都走过了」。
...
人为地记一步，不交给 AI。照常跑 rule 判据、列 human 闸门，末尾印进度条与下一步；`--note` 是这一步做了什么，省了按判据拼一句。
