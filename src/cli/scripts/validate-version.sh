...
#   Validates tag version matches Cargo.toml (and pyproject.toml if present).
#   Prints the validated version (without v prefix) on success.
#
#   （对齐 qtcloud-devops/src/cli/scripts/validate-version.sh）
...
# Validate pyproject.toml (optional; qtcloud-work cli 无 pyproject，跳过)
