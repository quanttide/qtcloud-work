...
- 文档与门禁补齐：
  - 接口参考分两类：`docs/api-references/commands/`（命令）与 `docs/api-references/concepts/`（概念）；`--help` 里的出处路径随之改。
  - 使用指南恢复「用例」一篇（五件真事），`validate-usecases.sh` 的对账不再空转。
  - `docs/dev-guide/layers.md` 按 `src/` 实际重写；新增 `docs/dev-guide/dependencies.md` 列明每个依赖与许可。
