/// 工作流（Workflow）这个模型的测试。
///
/// **怎么组织**：测试按 model 分文件，一个文件管一个模型。工作流的界面读法
/// （`models/workflow.dart`）与领域实现（`core/definition.dart`）都收在本文件里。
///
/// **期望从哪来**：以文档为事实源，不凭空造用例。
/// - 形状与校验：对着规格 `docs/specification/process/workflow.md`·语法逐条写；
/// - 界面模型：对着 `test/fixtures/` 里从命令行抓下来的真实输出写。
///
/// 每条测试的出处写在它上面一行（如 `// 规范：…`）；出处对不上就先改文档，再回填测试。
///
/// **分三层往下**：
/// 1. 模型怎么读——判据计数、工作流详情；
/// 2. 定义怎么读——读一份定义（校验）、一条定义怎么读（展示）；
/// 3. 动作怎么走——新建、导出、导入、核对。
library;
