...
void main() {
  group('命令怎么拼', () {
    test('每次都带上三处位置与 --json', () async {
      ...
      expect(runner.calls.single, [
        ...
        '/w',
        '--data',
        '/w/data/context/qtcloud-work',
        '--workflows',
        '/w/data/profile/quanttide/workflows',
        '--json',
        'task',
        '--list',
      ]);
    ...
    test('记一步带得上一句话', () async {
      ...
      expect(runner.calls.single.sublist(8), [
        'task',
        'demo',
        '--done',
        'audit',
    ...
    test('走下一步、记日志、核对定义各是各的命令', () async {
      ...
      expect(runner.calls[2].sublist(8), [
        'workflow',
        'devops-release',
        '--check',
      ]);
  ...
  group('读回模型', () {
    ...
    test('命令行说不行就抛，把它印的话带出来', () async {
      final runner = FakeRunner({
        'nope': '{"ok":false,"lines":["没有这条工作流：nope"]}',
      });
      final client = QtcloudWork(workspace: workspace, runner: runner);
      expect(
